...
def parse_dump(dumpfile, skipflag_data, zeromv_data, TrCoeff_data):
    reorder_map = gen_reorder_map()
    with open(dumpfile, "rt") as f:
        row_skipflg, row_zeromv, row_TrCoeff, frame_skipflag, frame_zeromv, frame_TrCoeff = [], [], [], [], [], []
        cur_row = 0
        for line in f:
            if line.find('mvinfo:POC') != -1:
                ...
                if len(row_TrCoeff) > 0: # last ctu row
                    frame_TrCoeff.append(row_TrCoeff)
                    row_TrCoeff, cur_row = [], 0
                ...
                if len(frame_TrCoeff) > 0:
                    TrCoeff_data.append(frame_TrCoeff)
                    cur_row, row_TrCoeff, frame_TrCoeff = 0, [], []
                ...
                if cur_row != y:
                    ...
                    frame_zeromv.append(row_zeromv)
                    frame_TrCoeff.append(row_TrCoeff)
                    row_skipflg, row_zeromv, row_TrCoeff = [], [], []
                    cur_row = y
            ...
            elif line.find('mvinfo:TrCoeff') != -1:
                tag1, tag2, w_str, h_str, data_str = line.split(':')
                w, h = int(w_str), int(h_str)
                data_str = data_str.split(',')
                data_int = np.array(data_str[0:w*h]).astype(np.uint16)
                data_reorder = reorder(data_int, reorder_map)
                data = data_reorder.reshape((w, h))
                row_TrCoeff.append(data)
...
def execute(dumpfile):
    ...
    zeromv_data = []
    TrCoeff_data = []
    parse_dump(dumpfile, skipflag_data, zeromv_data, TrCoeff_data)
    ...
    TrCoeff_maps = generate_maps(TrCoeff_data)
    TrCoeff_skip_maps = []
    for i in range(len(skip_maps)):
        zm = skip_maps[i] * TrCoeff_maps[i]
        TrCoeff_skip_maps.append(zm)

    #dump_maps(skip_maps, 'skip')
    #dump_maps(zeromv_maps, 'zeromv')
    dump_maps(zeromv_skip_maps, 'zeromv_skip')
    #dump_maps(TrCoeff_maps, 'trcoeff_data')
    #dump_maps(TrCoeff_skip_maps, 'trcoeff_skip')
